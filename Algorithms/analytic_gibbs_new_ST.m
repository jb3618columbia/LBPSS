...
for i=2:L
    ...
    [m, index] = max(prob_vec);
    place_holder = zeros(2*d,1);
    for z = 0:2*d -1
       place_holder(z+1,1) = prob_vec(mod(index+z-1, 2*d)+1, 1);
    ...
    if index == 1 
        ...
        for q=0:2*d-1
            v = u+q+1;
            delta = S(u+1) - S(v) + m;
            prob_vec_new(q+1,1) = max(0, min([delta, place_holder(u,1) + place_holder(v,1) - delta, place_holder(u,1), place_holder(v,1)]) );
